...
class TestModels(TestCase):
    ...
    def test_user_agreement(self):
        ...
        # simple agreement
        ua_u1_tos1 = UserAgreement.objects.create(terms_of_service=self.tos1, user=self.user1)
        ...
        self.assertEqual(
            str(ua_u1_tos1), f"{self.user1.username} agreed to TOS: {ua_u1_tos1.terms_of_service.created}: active"
        )
        ...
        ua_u1_tos2 = UserAgreement.objects.create(terms_of_service=self.tos2, user=self.user1)
        ua_u3_tos2 = UserAgreement.objects.create(terms_of_service=self.tos2, user=self.user3)
        ...
        self.assertEqual(
            str(ua_u1_tos1), f"{self.user1.username} agreed to TOS: {ua_u1_tos1.terms_of_service.created}: inactive"
        )
        self.assertEqual(
            str(ua_u1_tos2), f"{self.user1.username} agreed to TOS: {ua_u1_tos2.terms_of_service.created}: active"
        )
        self.assertEqual(
            str(ua_u3_tos2), f"{self.user3.username} agreed to TOS: {ua_u3_tos2.terms_of_service.created}: active"
        )
...
class TestManager(TestCase):
    def test_terms_of_service_manager(self):
        ...
        tos1 = TermsOfService.objects.create(content="first edition of the terms of service", active=True)
